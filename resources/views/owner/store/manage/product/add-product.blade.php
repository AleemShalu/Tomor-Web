                ...
                <form method="POST" action="{{ route('product.store') }}" enctype="multipart/form-data"
                    ...
                    <div>
                        <h2 class="text-2xl font-bold text-gray-900 mb-4">
                            <i class="fas fa-cube mr-2"></i> {{__('locale.product.page_product.create_product_title')}}
                        </h2>
                    </div>
                            ...
                            <div class="grid grid-cols-1 gap-3 mt-3">
                                ...
                                <div>
                                    ...
                                    <textarea name="translations[0][description]" id="en_description" rows="4"
                                              class="w-full px-4 py-2.5 border border-gray-300 rounded-lg focus:ring-primary-600 focus:border-primary-600"
                                    >{{ old('translations.0.description') }}</textarea>
                    ...
                    <div class="bg-gray-100 p-4 rounded-xl mb-6">
                        ...
                        <div class="relative mt-3">
                            <input type="file" name="product_images[]" id="product_images" accept=".png, .jpg, .jpeg"
                            ...
                            <span id="selected_image_count"
                                  class="text-sm text-gray-500 mt-1">0 image(s) selected</span>
                            <div class="grid grid-cols-3">
                                <div id="selected_images" class="mt-2"></div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="mt-6">
                        ...
                        <button type="submit"
                                class="mt-4 px-6 py-2 bg-blue-500 text-white font-medium rounded-lg hover:bg-blue-600">
                            Submit
                        </button>
                    </div>
                </form>
...
<script>
    // Add JavaScript to enforce the input format
    document.getElementById('unit_price').addEventListener('input', function (event) {
        let inputValue = event.target.value;
        // Remove any non-digit or non-period characters
        inputValue = inputValue.replace(/[^0-9.]/g, '');
        // Allow only one period (decimal point)
        inputValue = inputValue.replace(/(\..*)\./g, '$1');
        // Restrict to two decimal places
        inputValue = inputValue.replace(/(\.\d{2}).*/g, '$1');
